...
procSemantics CExprCall { _fname = "Coq_bind", _fparams = [arg] }   =
    error "Datatypes.Coq_bind with one arg found, undefined behavior."
procSemantics CExprCall { _fname = "Coq_bind", _fparams = [a, CExprLambda { .. }] } =
    CExprSeq (CExprStmt typenm nm a) $ procSemantics _lbody
...
procSemantics CExprCall { _fname = "Coq_bind", _fparams = a:b:arg } =
    error "Datatypes.Coq_bind with more than two args found!"
procSemantics CExprCall { _fname = "Coq_ret", .. }                  =
    CExprVar "return" -- TODO: implement return arguments
procSemantics CExprCall { .. }                                      =
    CExprCall _fname (map procSemantics _fparams)
procSemantics other                                                 =
    descend procSemantics other
